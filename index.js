...
app.put('/editbrand/:id', (req,res) => {
    ...
    conn.query(sql, (err, results) => {
        ...
        if(results.length > 0) {
            ...
            upload(req, res, (err) => {   //di eexecute setelah upload selesai.
                if(err){
                ...
                const { image } = req.files;
                // console.log(image)
                const imagePath = image ? path + '/' + image[0].filename : null;  // ? berarti (if) one line
                const data = JSON.parse(req.body.data);
                data.image = imagePath;
                ...
                try {
                    if(imagePath) {
                        ...
                        conn.query(sql,data, (err1,results1) => {
                            if(err1) {
                                fs.unlinkSync('./public' + imagePath); //apabila error, melakukan throwback, yaitu hapus file nya
                                return res.status(500).json({ message: "There's an error on the server. Please contact the administrator.", error: err1.message });
                            }
                            fs.unlinkSync('./public' + results[0].image); // hapus file yang akan di update
                            sql = `Select * from brand;`;
